...
// import imgOFF from "../../../assets/SmartLampOff.png"
// import imgOn from "../../../assets/SmartLampOnn.png"
...
const FeaturesSection = () => {
    ...
    return (
        <section className={`py-20 transition-all duration-500 ${isDarkMode ? 'bg-[#0A1128]' : 'bg-gray-50'}`}  dir={"ltr"}>
            <div className="max-w-7xl mx-auto px-4">
                {/* Header Section */}
                <div className="text-center mb-16">
                    <h2 className={`text-4xl lg:text-5xl font-bold mb-6 transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                ...
                {/* Features List Section */}
                <div className="mb-20">
                    <h3 className={`text-3xl font-bold text-center mb-12 transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>المميزات الرئيسية</h3>
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {features.map((feature, index) => (
                            <div
                                key={index}
                ...
                <div>
                    <div className="text-center mb-8">
                        <h3 className={`text-3xl font-bold mb-4 transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>معاينة مباشرة</h3>
                        <p className={`transition-colors duration-500 ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>شاهد تأثير التحكم على المصباح الذكي</p>
                    </div>
                    ...
                    <div className="mx-auto">
                        <div className={`relative overflow-hidden rounded-2xl transition-all duration-500 ${isLightOn
                            ...
                            <div className="relative w-full h-[500px]">
                                ...
                                {/* Light Image (Lazy loaded) - بدون فلاتر إضافية */}
                                <img
                                    src={imageUrls.light}
                                    alt="Smart Lamp On"
                                    className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${isLightOn ? 'opacity-100' : 'opacity-0'}`}
                                ...
                                {(!imagesLoaded.dark || (isLightOn && !imagesLoaded.light)) && (
                                    <div className={`absolute inset-0 animate-pulse flex items-center justify-center ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
                                        <div className={`text-lg transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>جاري التحميل...</div>
                                    </div>
                                )}
                            </div>

                            {/* Brightness Indicator - Fixed position for RTL */}
                            <div className="absolute top-6 right-6">
                                <div className={`px-6 py-3 rounded-full backdrop-blur-sm transition-all duration-300 ${isLightOn
                                        ? 'bg-white/80 text-gray-900'
                                        : isDarkMode
                            ...
                            {/* Status Indicator - Fixed position for RTL */}
                            ...
                            {/* Performance Indicator (Development only) - Fixed position for RTL */}
                            {process.env.NODE_ENV === 'development' && (
                                <div className="absolute top-6 left-6">
                                    <div className="px-3 py-1 bg-blue-500/80 text-white text-xs rounded">
                                        {imagesLoaded.dark && imagesLoaded.light ? '✓ Loaded' : '⏳ Loading'}
                    ...
                    <div className="max-w-2xl mx-auto">
                        <div className="rounded-xl">
                            <div className="flex items-center justify-between mb-6" dir="rtl">
                                <span className={`font-medium text-lg transition-colors duration-500 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>شدة الإضاءة</span>
                                <span className={`font-bold text-2xl transition-colors duration-500 ${isDarkMode ? 'text-[#00A4FF]' : 'text-[#1F3A93]'}`}>{brightness}%</span>
                            </div>
                            ...
                            <div className={`flex justify-between text-sm mb-6 transition-colors duration-500 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`} dir="rtl">
                                <span>ساطع</span>
                                <span>مطفأ</span>
                            </div>
                        </div>
